...
memberships: List[Dict] = [
    ...
    dict(user_tag = "user_one", project_tag="", role=orm.ProjectRoleEnum.admin),
]

def gen_persistent_ids():
    for company_tag, company in companies.items():
        company.id = id_from_tag(company_tag)
    for user_tag, user in users.items():
        user.id = id_from_tag(user_tag)
    for project_tag, project in projects.items():
        project.id = id_from_tag(project_tag)



async def insert(db_manager: DatabaseManager):
    async with db_manager.session(write=True) as session:
        for company_key, company_orm in companies.items():
            session.add(company_orm)
        for user_key, user_orm in users.items():
...
async def seed_database(db_manager: DatabaseManager):
    assert database_env.env_mode == "TEST", "db test seeding is allowed only in test environment"
    await drop_and_create(db_manager)
    await insert(db_manager)
